...
cl = LINETCR.LINE() #Tobirama
cl.login(token="EmJH5UF4OTYvxldwaZC9.nMM4Rh5fFP6pFB3iDCWOsq.HCAq/DkOBRemdEmnXU9cSNnTVIFP+T4rHUb0SBpOD38=")
cl.loginResult()

ki = LINETCR.LINE() #Minato
ki.login(token="EmZFnQHzgwxnnLHxUrP7.POnI+0U7i1Po/EMXkOsCrW.HZ32Kxl6cCIsZKGVOpmARsIg5Uzxt9aFG7rKQJzkF8Q=")
ki.loginResult()

kk = LINETCR.LINE() #Madara
kk.login(token="EmufescFwEsctjEYmaK1.7BIIrkQKLt7frT4XuBGpmq.68sWzAqYJveYNvW6ioolZRA+GbFx1oD+jm/+WztYayQ=")
kk.loginResult()

kc = LINETCR.LINE() #Hasirama
kc.login(token="EmZEURSyOH8mfyvaORaa.8oLSZ65oHzLtg/Q3XawOAG.C5SvXJjRGVOW3DcHUtuXuLHIU/Q9h9gY+GkSSVbD7RU=")
kc.loginResult()

ks = LINETCR.LINE() #Itama
ks.login(token="EmsM55TTedWN6i4hENI9.N1hSBiNfN+Z9DwLoF/h1Eq.jGwCfKB/AteOWt5S6UjFbd/fK4/f/0h0IlyYEvw2ZWQ=")
ks.loginResult()
...
Bots=[mid,Amid,Bmid,Cmid,Dmid,"u3669b8ac7e457e9e8f7dbdc875cc2849","ua68362ff4bbb235a4ef9e9b12a849c47","uedb8a244aaa4953ec546de3151b80f31","uf0e96ccfae4e8599531beab7c405a86a","u9f51ec6c89c59ffcfd75694e7fe1f579"]
admin=["u88cd7ee9fd691d6306e5e19be5d16f69"] 
owner=["u88cd7ee9fd691d6306e5e19be5d16f69"]
wait = {
    'contact':True,
    'autoJoin':True,
...
Minat? Silahkan PM!
Idline: http://line.me/ti/p/~prasetyo.86""",
    "lang":"JP",
...
#-----------------------------------------------
    ...
    #-------------Fungsi Tag All Finish---------------#
                ...
                if msg.toType == 2:
                    ...
                    for g in gs.members:
                        if _name in g.displayName:
                            targets.append(g.mid)
                    if targets == []:
                        random.choice(KAC).sendText(msg.to,"Jangan Add Kontak di atas")
                    else:
                        for target in targets:
        ...
        #-------------Fungsi Spam Start---------------------#
            elif msg.text in ["Up","up","Up Chat","Up chat","up chat","flood","Flood"]:
              if msg.from_ in admin:
                cl.sendText(msg.to,"▒░░░█▓▓▓▓█▓█▓▓▓▓▓█ ░▒░░")
                ki.sendText(msg.to,"▒░░░█▓▓▓▓█▓█▓▓▓▓▓█ ░▒░░")
                kk.sendText(msg.to,"▒░░░█▓▓▓▓█▓█▓▓▓▓▓█ ░▒░░")
...
#---------------------
        ...
        if op.type == 15:
           if op.param2 in Bots:
              return
           random.choice(KAC).sendText(op.param1, "Kok Left?\nBaper Kayaknya 😊 ")
